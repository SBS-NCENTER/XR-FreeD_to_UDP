...
- **네트워크 타임아웃 보호**: 도달 불가 타겟/DHCP 지연이 송출 카덴스를 막지 않도록 W5500 재전송·DHCP 타임아웃 단축
...
| 네트워크 순단 | ✅ 링크 모니터링, DHCP 갱신, 도달 불가 타겟 타임아웃 단축(송출 비차단) |
...
| UDP 전송 지연 | < 1ms |
| 최대 타겟 수 | 4개 |
| RAM 사용량 | ~5KB (빌드 실측, 32KB 중 15.8%) |
| 워치독 타임아웃 | ~5.59초 (RA4M1 WDT HW 최대) |
| W5500 재전송 타임아웃 | 20ms × 1 (도달 불가 타겟 ~40ms 내 포기) |
...
| 1.3 | 2026-06 | 하드웨어 워치독 실제 구현(~5.59초), W5500 재전송·DHCP 타임아웃 단축으로 카덴스 안정화 |
